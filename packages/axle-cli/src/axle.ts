...
import { scanUrl, type AxeViolation, type ScanResult } from "./scanner.js";
import { generateFix, type FixResult } from "./fixer.js";
...
function parseArgs(argv: string[]): { command: string | null; args: Args } {
...
async function main() {
  ...
  await writeFile(
    args.jsonOut,
    JSON.stringify(
      { result, fixes: Object.fromEntries(fixes) },
      null,
      2
    )
  );
