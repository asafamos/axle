...
import type { AxeViolation } from "./scanner";
...
- Output must match this TypeScript type exactly:
  {
    "explanation": string,  // one sentence, plain English, explains the root cause
    "fix_strategy": string,  // one sentence, explains WHAT to change
    "patches": Array<{
      "selector": string,       // the CSS selector of the element to modify
      "before": string,         // current HTML (outerHTML) as given in the input
      "after": string,          // fixed HTML (outerHTML)
      "notes": string           // optional guidance for human reviewer, can be empty
    }>,
    "confidence": "high" | "medium" | "low",  // how confident AI is this fix is correct
    "manual_review_needed": boolean   // true if this fix requires human decisions (e.g., choosing alt text for brand imagery)
  }
...
Produce a JSON fix following the schema in the system prompt. Return only valid JSON.`;
  ...
  // Strip potential markdown fences if model slipped
  const cleaned = text
    .replace(/^```json\s*/i, "")
    .replace(/^```\s*/i, "")
    .replace(/\s*```\s*$/i, "")
